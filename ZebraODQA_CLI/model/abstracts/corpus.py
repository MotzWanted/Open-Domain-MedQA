...
class Corpus(ABC):
    ...
    def __init__(self, tokenizer):
        ...
        nltk.download("punkt", download_dir=config_cache_dir)
        self._sentence_splitter = nltk.data.load(
            os.path.join(config_cache_dir, "tokenizers/punkt/english.pickle"))
        self.counter = 0
    ...
    def index_answers(self, sentences, answer):
        ...
        for sentence in sentences:
            il = []
            for i in range(
                    len(self._tokenizer.tokenize(sentence["sentence"])) - len(self._tokenizer.tokenize(answer)) + 1):
                if self._tokenizer.tokenize(sentence["sentence"])[
                   i:i + len(self._tokenizer.tokenize(answer))] == self._tokenizer.tokenize(answer):
