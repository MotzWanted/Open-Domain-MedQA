...
class QATrainer():
    ...
    def _training(self):
        ...
        for step, batch in enumerate(self.train_data):
            ...
            # ).to(device)
            try:
                inputs = {"input_ids": torch.tensor([batch["input_ids"]]).to(config_device), "attention_mask": torch.tensor([batch["attention_mask"]]).to(Config.device)}
    ...
    def _validation(self):
        ...
        for step, batch in enumerate(self.validation_data):
            ...
            try:
                ...
                # MedQA accuracy
                total_eval_medqa_accuracy += self.medqa.score(
                    {"id": batch["id"], "prediction_text": golden_candidate},
                    {"id": batch["id"], "answers": batch['answers']}
                )["exact_match"] / 100
        ...
        # SQUAD - Report the final accuracy for this validation run.
        #avg__eval_squad_accuracy = total_eval_squad_accuracy / dataset_len
        #print("\n  Squad Accuracy: {0:.2f}".format(avg__eval_squad_accuracy))

        # MEDQA - Report the final accuracy for this validation run.
        avg__eval_medqa_accuracy = total_eval_medqa_accuracy / dataset_len
        print("\n  MedQA Accuracy: {0:.2f}".format(avg__eval_medqa_accuracy))
        ...
        return avg__eval_medqa_accuracy, avg_eval_index_accuracy, avg_val_loss, validation_time
    ...
    def train(self):
        for epoch_i in range(0, self.epochs):
            ...
            avg__eval_medqa_accuracy, avg_eval_index_accuracy, avg_val_loss, validation_time = self._validation()
            ...
            self.training_stats.append(
                {
                    ...
                    "Valid. MedQA Accur.": avg__eval_medqa_accuracy,
