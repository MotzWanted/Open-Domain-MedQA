...
from transformers import BertPreTrainedModel, BertModel, BertConfig, BertTokenizer, BertTokenizerFast
from transformers.models.bert.modeling_bert import BertOnlyMLMHead, ACT2FN

class REALMEmbedder(BertPreTrainedModel, Embedder):
    def __init__(self, config):
        super().__init__(config)

        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') # BertTokenizerFast("../Bert/assets/vocab.txt")
    ...
    def embed_context(self, corpus) -> Corpus:
        return corpus.map(lambda example: {'embeddings': self(**self.tokenizer(example["context"], padding=True, truncation=True, return_tensors="pt").to(config_device))[0].cpu().detach().numpy()}, batched=True, batch_size=5)
    ...
    def load(config, checkpoints_path = None):
        if(config is None):
            config = BertConfig()
        cache_path = os.path.join(config_cache_dir, "embedder.pth.tar")
