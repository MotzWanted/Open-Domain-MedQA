...
index_name = 'docs_bm25_shards5_new'
...
textbooks = [f for f in listdir(text_path) if ".pkl" in f]
sentence_list = []
for book in textbooks:
    with open(text_path + "/" + book,"rb") as file:
        print(book)
        textbook = pickle.load(file)
    filt_list = [el for el in textbook if len(el.split())==100]
    sentence_list.extend(filt_list)
...
ingest_docs(sentence_list, index_name)
